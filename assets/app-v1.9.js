(() => {
  ...
  function renderBookingOverview(id) {
    ...
    currentBookingId=id;
    const c=(state.customers||[]).find(x=>x.id===b.customerId)||{};
    const invoices=(state.invoices||[]).filter(i=>i.bookingId===b.id);
    const depositInvoice=invoices.find(i=>i.type==='Deposit'&&i.status!=='Cancelled');
    const paid=bookingPaidAmount(b.id);
    const total=Number(b.grandTotal??b.total??0);
    ...
    setHeader(b.title||'Booking','BOOKING OVERVIEW');
    view.innerHTML=`
      <div class="booking-overview-nav">
        <button class="soft-btn" id="bookingBack">← All bookings</button>
        <button class="primary-btn" id="bookingEdit">Edit booking</button>
      </div>
      <div class="card booking-hero">
        <div>
          <div class="eyebrow">${esc(dateFmt(b.eventDate).toUpperCase())}</div>
          <h2>${esc(b.title||'Untitled booking')}</h2>
          <p>${esc(b.customerName||customerDisplayName(c))}${b.venue?` · ${esc(b.venue)}`:''}</p>
        </div>
        <div class="booking-hero-status">${badge(b.status)} ${badge(`Contract: ${b.contractStatus||'Draft'}`)}</div>
      </div>

      <div class="workflow-strip section">
        <div class="workflow-step"><span>1</span><div><b>Contract</b><small>${esc(b.contractStatus||'Draft')}${b.contractSentAt?` · sent ${dateFmt(b.contractSentAt)}`:''}</small></div></div>
        <div class="workflow-step"><span>2</span><div><b>Deposit invoice</b><small>${depositInvoice?`${esc(depositInvoice.number)} · ${esc(depositInvoice.status)}`:'Not created'}</small></div></div>
        <div class="workflow-step"><span>3</span><div><b>Booking</b><small>${esc(b.status||'Date Held')}</small></div></div>
      </div>

      <div class="card panel section">
        <div class="section-head"><div><h2>Booking actions</h2><p>Move this booking through the workflow</p></div></div>
        <div class="booking-action-grid">
          <button class="workflow-action" id="sendContract"><b>✉ Send contract</b><span>${b.contractStatus==='Accepted'?'Resend client copy':'Issue terms to the client'}</span></button>
          <button class="workflow-action" id="depositInvoice"><b>£ ${depositInvoice?'Deposit invoice':'Create deposit invoice'}</b><span>${depositInvoice?`${depositInvoice.number} · ${depositInvoice.status}`:`Create invoice for ${money(b.deposit)}`}</span></button>
          <button class="workflow-action" id="sendConfirmation"><b>✓ Send booking confirmation</b><span>${b.confirmationSentAt?`Last sent ${dateFmt(b.confirmationSentAt)}`:'Email the confirmed booking details'}</span></button>
          <button class="workflow-action" id="recordBookingPayment"><b>＋ Record payment</b><span>Record money against an invoice</span></button>
          <button class="workflow-action" id="addBookingService"><b>◇ Add service</b><span>Add extras to this booking</span></button>
          <button class="workflow-action" id="addBookingNote"><b>✎ Add note</b><span>Add an internal booking note</span></button>
          <a class="workflow-action" href="${esc(portal)}" target="_blank" rel="noopener"><b>↗ Client portal</b><span>Contract and invoices for the client</span></a>
        </div>
      </div>

      <div class="grid cols-2 section booking-detail-grid">
        <div class="card panel">
          <div class="section-head"><div><h2>Client</h2><p>The customer permanently linked to this booking</p></div></div>
        ...
        <div class="card panel">
          ...
          <div class="overview-list">
            ...
            <div><span>Venue address</span><b class="preline">${esc(b.venueAddress||b.venuePostcode||'—')}</b></div>
      ...
      <div class="grid cols-2 section booking-detail-grid">
        <div class="card panel">
          <div class="section-head"><div><h2>Financials</h2><p>Booking value and money received</p></div></div>
        ...
        <div class="card panel">
          <div class="section-head"><div><h2>Additional services</h2><p>Extras added after the main entertainment package</p></div><button class="action-link" id="addBookingServiceSmall">＋ Add</button></div>
          ${additional.length?`<div class="overview-lines">${additional.map(x=>`<div><div><b>${esc(x.name||x.description||'Service')}</b><small>${esc(x.quantity||1)} × ${money(x.unitPrice||0)}</small></div><div><b>${money(Number(x.quantity||1)*Number(x.unitPrice||0))}</b><button class="action-link" data-remove-service="${x.id}">Remove</button></div></div>`).join('')}</div>`:empty('No additional services','Use Add service to include extras.')}
        </div>
      </div>

      <div class="card table-card section">
        <div class="table-toolbar"><div><b>Invoices</b><div class="cell-sub">Invoices linked to this booking</div></div><button class="outline-btn" id="createAnyInvoice">＋ Create invoice</button></div>
        ${invoices.length?table(['Invoice','Type','Due','Amount','Paid','Status',''],invoices.map(i=>`<tr><td><b>${esc(i.number)}</b></td><td>${esc(i.type||'Invoice')}</td><td>${dateFmt(i.dueDate)}</td><td class="money">${money(i.amount)}</td><td class="money">${money(invoicePaidAmount(i.id))}</td><td>${badge(i.status)}</td><td><button class="action-link" data-open-invoice="${i.id}">View</button></td></tr>`)):empty('No invoices yet','Create the deposit invoice when you are ready to bill the client.')}
      </div>

      <div class="card panel section">
        <div class="section-head"><div><h2>Internal notes</h2><p>Notes for this booking only</p></div><button class="action-link" id="addBookingNoteSmall">＋ Add note</button></div>
    ...
    $('#depositInvoice').onclick=()=>createDepositInvoice(id);
    $('#sendConfirmation').onclick=()=>sendBookingMessage(id,'confirmation');
    $('#recordBookingPayment').onclick=()=>recordBookingPayment(id);
    ...
    $('#createAnyInvoice').onclick=()=>openInvoiceCreate(id);
  ...
  function openBooking(id=null){
    ...
    openModal(id?'Edit booking':'New booking','EVENT',`<div class="booking-form" style="display:block!important;width:100%!important;">
      ...
      if(!selectedCustomer)return toast('Choose an existing customer before saving the booking');
  ...
  async function createDepositInvoice(bookingId){
    ...
    const existing=(state.invoices||[]).find(i=>i.bookingId===bookingId&&i.type==='Deposit'&&i.status!=='Cancelled');
    if(existing){ toast(`Deposit invoice ${existing.number} already exists`); openInvoice(existing.id); return; }
    const amount=Number(b.deposit||0); if(amount<=0)return toast('This booking has no deposit amount');
    if(mode==='api'){
      await api(`/api/bookings/${bookingId}/invoice`,{method:'POST',body:JSON.stringify({type:'Deposit',amount,description:`Deposit - ${b.title}`})}); await refresh();
    } else {
      const number=`${state.settings.invoicePrefix||'INV-'}${state.settings.nextInvoiceNumber||1}`; state.settings.nextInvoiceNumber=Number(state.settings.nextInvoiceNumber||1)+1;
      const due=new Date(); due.setDate(due.getDate()+Number(state.settings.paymentTermsDays||14));
      state.invoices.unshift({id:uid('inv'),createdAt:isoNow(),number,bookingId:b.id,customerId:b.customerId,customerName:b.customerName,type:'Deposit',issueDate:ymd(),dueDate:ymd(due),amount,status:'Draft',paidAt:null,description:`Deposit - ${b.title}`});
      addActivity('invoice',`Deposit invoice ${number} created for ${b.title}`); saveLocal();
    }
    toast('Deposit invoice created'); renderBookingOverview(bookingId);
  }
  ...
  function sendBookingMessage(bookingId,type){
    ...
    if(isContract && (mode!=='api' || !portalIsExternallyReachable(portal))){
      openModal('Contract email not ready','CLIENT PORTAL',`<div class="notice" style="margin-bottom:16px"><b>Do not email this contract link to a customer yet.</b><br><br>${mode!=='api'?'This CRM is currently running in <b>Static demo mode</b>. The booking only exists in this browser, so another person cannot retrieve or accept it.':'The Node server is running on a local/private address. A customer outside your computer/network cannot open that portal URL.'}</div><div class="field"><label>LOCAL CONTRACT PREVIEW</label><input value="${esc(portal)}" readonly></div><div class="helper" style="margin-top:12px">You can preview the contract now. To send live contracts that update the CRM automatically, deploy the supplied Node application to a public HTTPS address (for example Render/Railway/VPS) and open the CRM from that address.</div>`, 'Preview contract',()=>{window.open(portal,'_blank','noopener');modal.close();});
      return;
    }
    const subject=isContract?`Booking contract – ${b.title}`:`Booking confirmation – ${b.title}`;
    const body=isContract?`Hi ${first},\n\nPlease review your booking details and contract for ${b.title} on ${dateFmt(b.eventDate)}.\n\nOpen your client portal here:\n${portal}\n\nOnce you accept the terms, the booking will be updated automatically.\n\nThanks,\n${state.settings.businessName||'EventFlow'}`:`Hi ${first},\n\nHere are the confirmed booking details for ${b.title} on ${dateFmt(b.eventDate)}.\n\nVenue: ${b.venueName||b.venue||'TBC'}\nStart: ${b.startTime||'TBC'}\n${b.noFinishTime?'Finish: No fixed finish time':`Finish: ${b.finishTime||b.endTime||'TBC'}`}\nTotal: ${money(b.total)}\n\nYou can view the booking and invoices here:\n${portal}\n\nThanks,\n${state.settings.businessName||'EventFlow'}`;
    openModal(isContract?'Send contract':'Send booking confirmation','CLIENT EMAIL',`<div class="form-grid"><div class="field full"><label>TO</label><input id="msgTo" type="email" value="${esc(c.email||'')}"></div><div class="field full"><label>SUBJECT</label><input id="msgSubject" value="${esc(subject)}"></div><div class="field full"><label>MESSAGE</label><textarea id="msgBody" style="min-height:260px">${esc(body)}</textarea></div>${mode==='local'?`<div class="field full"><div class="notice">Static demo mode cannot send mail directly. Booking confirmations can be prepared in your normal email application. Live contract links require the shared Node/server version so the customer can retrieve and accept the booking.</div></div>`:''}</div>`,mode==='api'?'Send email':'Prepare email',async()=>{
      const to=val('#msgTo'), subj=val('#msgSubject'), text=val('#msgBody'); if(!to)return toast('Customer email address is required');
      try{
        if(mode==='api') await api('/api/send-email',{method:'POST',body:JSON.stringify({to,subject:subj,text})});
        const patch=isContract?{status:b.status==='Confirmed'?'Confirmed':'Contract Issued',contractStatus:b.contractStatus==='Accepted'?'Accepted':'Sent',contractSentAt:isoNow(),contractSentTo:to}:{confirmationSentAt:isoNow(),confirmationSentTo:to};
        if(mode==='api'){
          await api(`/api/bookings/${bookingId}`,{method:'PUT',body:JSON.stringify(patch)});
          if(isContract) await api('/api/documents',{method:'POST',body:JSON.stringify({bookingId,type:'Contract',status:'Sent',sentAt:isoNow(),sentTo:to,portalUrl:portal})});
          await refresh();
        }else{
          Object.assign(b,patch,{updatedAt:isoNow()}); if(isContract){state.documents||=[];state.documents.unshift({id:uid('doc'),createdAt:isoNow(),bookingId,type:'Contract',status:'Prepared',sentTo:to,portalUrl:portal});}
          addActivity(isContract?'contract':'email',`${isContract?'Contract':'Booking confirmation'} prepared for ${b.title}`); saveLocal();
          window.open(`mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subj)}&body=${encodeURIComponent(text)}`,'_self');
        }
        modal.close(); toast(isContract?'Contract issued':'Booking confirmation sent/prepared'); renderBookingOverview(bookingId);
      }catch(err){ toast(err.message||'Email could not be sent'); }
    });
  }
