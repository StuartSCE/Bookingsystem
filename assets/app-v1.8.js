(() => {
  ...
  function bookingPortalLink(id) {
    ...
    try { return new URL(relative, location.href).href; } catch { return relative; }
  }
  ...
  function sendBookingMessage(bookingId,type){
    ...
    openModal(isContract?'Send contract':'Send booking confirmation','CLIENT EMAIL',`<div class="form-grid"><div class="field full"><label>TO</label><input id="msgTo" type="email" value="${esc(c.email||'')}"></div><div class="field full"><label>SUBJECT</label><input id="msgSubject" value="${esc(subject)}"></div><div class="field full"><label>MESSAGE</label><textarea id="msgBody" style="min-height:260px">${esc(body)}</textarea></div>${mode==='local'?`<div class="field full"><div class="notice">Static GitHub/demo mode cannot send mail directly. This will open your normal email application with the message filled in and record the workflow action in the CRM. The Node server can send it automatically once SMTP is configured.</div></div>`:''}</div>`,mode==='api'?'Send email':'Prepare email',async()=>{
