(() => {
  ...
  const DEMO_SEED = {
    ...
    customers:[{id:'cus_1',createdAt:'2026-03-11T09:00:00Z',name:'Emma & Jack Wilson',company:'',email:'emma@example.com',phone:'07700 910101',address:'Chichester, West Sussex',source:'Referral',notes:'Prefers email'},{id:'cus_2',createdAt:'2026-04-02T09:00:00Z',name:'Harbour Events Ltd',company:'Harbour Events Ltd',email:'events@harbour.example.com',phone:'023 9000 1000',address:'Portsmouth, Hampshire',source:'Google',notes:'Corporate client'}],
  ...
  function renderCustomers() {
    ...
    const rows = (state.customers||[]).map(c=>{const count=(state.bookings||[]).filter(b=>b.customerId===c.id).length; const value=(state.bookings||[]).filter(b=>b.customerId===c.id).reduce((s,b)=>s+Number(b.total||0),0);return `<tr data-search="${esc(JSON.stringify(c).toLowerCase())}"><td><div class="cell-title">${esc(c.name)}</div><div class="cell-sub">${esc(c.company||'')}</div></td><td>${esc(c.email||'—')}<div class="cell-sub">${esc(c.phone||'')}</div></td><td>${esc(c.address||'—')}</td><td>${esc(c.source||'—')}</td><td>${count}</td><td class="money">${money(value)}</td><td><button class="action-link" data-edit-customer="${c.id}">Edit</button></td></tr>`});
    view.innerHTML = listPage('Customers','Keep repeat clients, companies and contact details in one place.','New customer','customer',['Customer','Contact','Location','Source','Bookings','Lifetime value',''],rows);
